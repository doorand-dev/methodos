...
> Methodos 분산 게이트가 산출하는 *measurable artifact* 표준. Claude와 Codex의
> reviewer topology가 다르므로 아래 runtime ownership을 함께 읽는다.
...
`<plan_root>/<slug>.md` — Markdown frontmatter + body. impl/final reviewer와 Claude
plan reviewer가 *본문 paste 받음* (self-contained 필수).
...
slices:
  - id: 1
    ...
    checkpoint_reason: null | schema_or_public_contract | authority_or_security | persistence_or_idempotency | migration_or_external_state | financial_execution | downstream_foundation
...
- `slices[].review_checkpoint`: 기본 `skip`. schema/public contract, authority/security,
  persistent artifact/latest/idempotency/concurrency, migration/external state,
...
> `verification.type` 값은 구현자와 final reviewer가 재현할 실행법을 제공한다.
...
## Agent 산출 schema 종합 (2026-05-23 reviewer agent promote, OMC 통합 패턴)
...
Every repair report must carry the same `owner_thread_or_session` value as the
attempt-1 implementation report. A different or missing owner identity is a
mechanical seam mismatch and cannot be accepted as the same owner.
...
`fallback_reason`은 shared schema 호환을 위해 유지하지만 자동 fallback이 없는
Codex route에서는 null이다.
...
`<verify_root>/<review-runtime>-impl-<slug>.json` — 사용자가 명시 요청했을 때만 `/impl` 컨트롤러가 다른 런타임의 최종 diff review를 **foreground+timeout 단일 호출**로 1회 실행하고 결과를 저장한다. Claude에서 Codex companion을 쓰는 경우 기존 파일명은 `<verify_root>/codex-impl-<slug>.json`이고 `kind`는 `"codex-impl"`일 수 있다. Codex runtime은 Codex를 다시 부르지 말고, 사용자 명시 요청과 별도 reviewer runtime 가용성이 모두 있을 때만 이 advisory를 만든다.
...
**활성화 (단일 predicate — 모든 자리 동일하게)**: 사용자가 현재 작업에서 별도
reviewer runtime의 advisory review를 명시적으로 요청했을 때만 1회 실행·artifact를
기록한다. 작업 크기, D33 충족, reviewer runtime 설정만으로 자동 호출하지 않는다.

각 schema 본문은 해당 agent md의 `<Output_Format>` 섹션에 inline. drift 방지 룰: agent md 변경 시 이 SKILL-ARTIFACTS의 schema도 같이 갱신 (또는 그 반대).

### plan-verify schema (kind: "plan-verify", v1.4 — lineage/scoped/provider route)
...
- `reviewer_mode`가 `fresh_web_session`, `fresh_subagent`, 또는
  `fresh_external_session`이 아니거나
  `reviewer_role=impl-verify-reviewer`가 없으면 DONE/DONE_WITH_CONCERNS를 쓰지
  않는다. reviewer dispatch 불가도 `NEEDS_CONTEXT`다.
...
- 아래 lineage, model/effort 상속, scoped/full escalation 규칙은 Claude
  `impl-verify` realization에만 적용한다. Codex automatic lifecycle에는
  `impl-verify`를 사용하지 않는다.
- Claude `impl-verify`는 같은 `approved_plan_revision`과 slice에서 prior issue closure를
  위해 이어진 `parent_candidate_sha → candidate_sha`만 같은 candidate lineage로
  본다. plan revision이 바뀌면 attempt 1의 새 lineage로 시작한다.
- Claude `impl-verify` attempt 1은 `review_scope=full`,
  `parent_candidate_sha=null`이다. attempt M+1은 기본 `review_scope=scoped`이며 parent
  SHA가 직전 attempt candidate와 일치해야 한다.
- Claude `impl-verify` attempt 2+ scoped는 해당 Claude route의 parent session
  model/effort를 상속한다. reviewer를 실행할 수 없으면 `NEEDS_CONTEXT`다.
- Claude `impl-verify`의 `reviewer_provider`, `reviewer_transport`, `reviewer_model`,
  `reviewer_reasoning_effort`는 실제 dispatch 값을 기록한다. runtime이 상속된 실제
  값을 노출하지 않으면 model/effort에 `inherited_from_parent`를 기록할 수 있다.
- Claude `impl-verify` attempt M+1의 `review_scope=full`은 `escalation_reason`이
  ...
  때만 유효하다.
- Claude `impl-verify` scoped reviewer가 위 predicate로 `NEEDS_CONTEXT`를 반환한 응답은 routing
  envelope이며 terminal attempt artifact로 저장하지 않는다. 같은 attempt/candidate/
  parent의 full 결과 하나만 저장한다. 다른 `NEEDS_CONTEXT`는 terminal이다.

### impl-checkpoint schema (kind: "impl-checkpoint", Codex v1.0)

`<verify_root>/checkpoint-<slug>-slice-<N>-attempt-<M>.json`은 Codex가 명시적
high-risk slice에만 만든다. 일반 slice에는 만들지 않는다.

```json
{
  "schema_version": "1.0",
  "kind": "impl-checkpoint",
  "target": "<slug>",
  "slice_id": "<N>",
  "attempt": 1,
  "approved_plan_revision": "<approved plan SHA>",
  "base_ref": "<slice base SHA>",
  "candidate_sha": "<slice candidate SHA>",
  "parent_candidate_sha": null,
  "review_scope": "full | scoped",
  ...
  "stage_results": {
    "requirements": "PASS | FAIL | NEEDS_CONTEXT | SKIPPED",
    "impact": "PASS | FAIL | NEEDS_CONTEXT | SKIPPED",
    "quality": "PASS | FAIL | NEEDS_CONTEXT | SKIPPED",
    "verification": "PASS | FAIL | NEEDS_CONTEXT | SKIPPED"
  },
  "evidence": [
    {"command": "<fresh command>", "output_excerpt": "<actual output>", "interpretation": "<linked invariant result>"}
  ],
  "findings": [
    {
      "issue_id": "<stable id>",
      "stage": "requirements | impact | quality | verification",
      "grade": "critical | important | regression | deferred_decision | polish",
      "where": "<file:line>",
      "backlink_type": "acceptance_criterion | user_story | public_invariant | none",
      "backlink": "<exact approved reference | null>",
      "observable_invariant": "<required behavior, not implementation>",
      "what": "<expected vs actual>",
      "route_to": "gate | todos",
      "recommend": "<invariant-focused fix direction>"
    }
  ],
  "escalation_required": false,
  "escalation_reason": null,
...
- gating finding은 정확한 approved acceptance criterion, user story, explicit
  public invariant backlink가 필수다. `backlink_type=none`이면 `polish` 또는
  `deferred_decision`이며 `route_to=todos`다.
- reviewer는 관찰 가능한 invariant와 이를 증명하는 최소 oracle만 요구한다. 특정
  구현법이나 무관한 coverage는 요구하지 않는다.

### impl-narrative-final schema (kind: "impl-narrative-final", Claude v1.2 / Codex v1.4)

```json
{
  "schema_version": "1.2 | 1.4",
  ...
  "stage_results": {
    ...
    "quality": "PASS | FAIL | NEEDS_CONTEXT | SKIPPED",
    "verification": "PASS | FAIL | NEEDS_CONTEXT | SKIPPED",
    "narrative": "PASS | FAIL | NEEDS_CONTEXT"
  },
  "terminal_regression": {
    "candidate_sha": "<final candidate SHA>",
    "command": "<actual command | NOT_DECLARED>",
    "exit_code": "<0 | null>",
    "output_excerpt": "<actual output | residual scope>"
  },
  "escalation_required": false,
  "escalation_reason": null,
  "narrative": [
  ...
  "findings": [
    {
      "issue_id": "<stable id>",
      "stage": "requirements | impact | quality | verification | narrative",
      "grade": "critical | important | broken | regression | deferred_decision | polish",
      "where": "<file:line>",
...
- Codex v1.4에서 gating finding은 정확한 approved acceptance criterion, user story,
  또는 explicit public invariant backlink가 필수다. backlink가 없으면 동작하는
  `polish`/`deferred_decision`으로만 분류하고 통과시킨다. Reviewer는 관찰 가능한
  invariant와 그 최소 oracle을 요구하며 특정 구현법이나 무관한 test coverage를
  강제하지 않는다.
- Codex v1.4에서 `critical`, `broken`, `regression`, failed terminal regression은
  BROKEN이다. `important`는 DONE_WITH_CONCERNS 상태를 새로 만들지 않고 기본적으로
  BROKEN으로 닫되, 동작하는 polish만 `polish`/todos로 통과시킨다.
...
- 공통: 슬라이스마다 `WHY:` commit 존재
- Claude realization: plan-verify와 slice attempt artifact의 latest status 확인
