...
`runtime-notes/codex.md`가 정본이며 full은 fresh ChatGPT Pro/extended 웹 세션
primary + 결과 자체를 얻지 못한 transport/finality 실패에만 fresh local
`gpt-5.6-sol/xhigh` fallback, scoped는 fresh local `gpt-5.6-sol/medium`이다.
프로젝트 machine route가 있으면 dispatch 직전 point-of-use로 다시 읽고
provider/model/effort를 모두 명시한다. 이전 reviewer/controller 값이나 runtime
default를 상속하지 않는다.
...
- Pro가 `BLOCKED`, `NEEDS_CONTEXT`, 또는 issue를 반환한 것은 성공한 review다.
  local full fallback은 결과 자체를 얻지 못한 5개 transport/finality 사유에서만
  허용하고 `fallback_reason`에 기록한다.
- `reviewer_provider`/`reviewer_transport`/`reviewer_model`/
  `reviewer_reasoning_effort`는 실제 dispatch 값이다. inherited/default/unknown 값으로
  DONE/DONE_WITH_CONCERNS를 쓰지 않는다.
...
- Pro가 `BLOCKED`, `NEEDS_CONTEXT`, 또는 issue를 반환한 것은 성공한 review다.
  ...
  허용하고 `fallback_reason`에 기록한다.
- `reviewer_provider`, `reviewer_transport`, `reviewer_model`,
  `reviewer_reasoning_effort`는 dispatch에 실제 명시한 값이다. inherited/default/
  unknown 값으로 DONE/DONE_WITH_CONCERNS를 쓰지 않는다.
...
- scoped reviewer가 위 predicate로 `NEEDS_CONTEXT`를 반환한 응답은 routing
  envelope이며 terminal attempt artifact로 저장하지 않는다. 같은 attempt/candidate/
  parent의 full 결과 하나만 저장한다. 다른 `NEEDS_CONTEXT`는 terminal이다.
- Pro가 `BROKEN`, `NEEDS_CONTEXT`, 또는 issue를 반환한 것은 성공한 review다.
  local full fallback은 결과 자체를 얻지 못한 5개 transport/finality 사유에서만
  허용하고 `fallback_reason`에 기록한다.
- DONE은 fresh evidence, closed impact/flow selector, explicit provider/transport/
  reviewer model/effort가 모두 있어야 한다. inherited/default/unknown 값은 허용하지
  않는다.
