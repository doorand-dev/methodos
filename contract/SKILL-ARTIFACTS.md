...
`runtime-notes/codex.md`가 정본이며 final attempt 1 full은 현재 부모 세션의
model/effort를 상속한 fresh local read-only subagent, failed-review repair의 attempt
2+ scoped는 fresh local `gpt-5.6-sol/medium`이다.
...
- Codex 기본 full route는 부모 session model/effort를 상속한 fresh local
  subagent다. local reviewer를 실행할 수 없으면 자동 외부 fallback 없이
  `NEEDS_CONTEXT`다. 외부 provider는 사용자 명시 요청 때만 쓴다.
- `reviewer_provider`/`reviewer_transport`/`reviewer_model`/
  `reviewer_reasoning_effort`는 실제 dispatch 값이다. runtime이 상속된 실제 값을
  노출하지 않는 local full route는 두 값에 `inherited_from_parent`를 기록할 수 있다.
  이는 unknown/default가 아니라 custom-agent 설정 생략에 따른 명시적 provenance다.
...
### impl-narrative-final schema (kind: "impl-narrative-final", Claude v1.2 / Codex v1.3)
...
- Claude v1.2의 full escalation은 Claude realization이 정한다. Codex v1.3은 attempt
...
- Codex 기본 full route는 부모 session model/effort를 상속한 fresh local
  subagent다. local reviewer를 실행할 수 없으면 자동 외부 fallback 없이
  `NEEDS_CONTEXT`다. 외부 provider는 사용자 명시 요청 때만 쓴다.
- Codex v1.3의 `stage_results`와 `terminal_regression`은 필수다. Claude v1.2에서는
  ...
  모두 있어야 한다. runtime이 상속된 실제 값을
  노출하지 않는 local full route는 model/effort에 `inherited_from_parent`를 기록할 수
  있다.
- Codex attempt 2+ scoped DONE은 selected stage PASS와 unaffected stage SKIPPED를
  허용하며, 각 SKIPPED에는 selector 밖이라는 이유가 있어야 한다.
- Codex v1.3에서 `critical`, `broken`, `regression`, failed terminal regression은
...
- Codex realization: plan-verify와 slice attempt artifact를 요구하지 않음
- Codex 골 종료: latest narrative-final v1.3 status DONE, 모든 stage PASS,
