...
| `<verify_root>/<review-runtime>-impl-<slug>.json` | 사용자 명시 요청형 cross-runtime advisory review | 모든 슬라이스 impl-verify와 최종 novelist 통과 후 1회, loop 없음 |
...
`runtime-notes/codex.md`가 정본이며 full은 현재 부모 세션의 model/effort를 상속한
fresh local read-only subagent, scoped는 fresh local `gpt-5.6-sol/medium`이다.
...
### impl-narrative-final schema (kind: "impl-narrative-final", v1.2)
...
- Codex 기본 full route는 부모 session model/effort를 상속한 fresh local
  subagent다. local reviewer를 실행할 수 없으면 자동 외부 fallback 없이
  `NEEDS_CONTEXT`다. 외부 provider는 사용자 명시 요청 때만 쓴다.
- DONE은 fresh evidence, closed impact/flow selector, explicit provider/transport/
  reviewer model/effort provenance가 모두 있어야 한다. runtime이 상속된 실제 값을
