...
- 부모 턴을 닫아야 하는 장시간 작업, 사용자 표시가 필요한 작업, 세션 경계를 넘는
  위임만 독립 thread로 만든다. 독립 thread 발주에는 전역 AGENTS.md의 회신 계약을
...
회수는 두 모드 중 하나를 발주 시점에 정한다.

- bounded wait: 발주자가 `wait_threads`로 대기한다. 대기 상한이 짧으므로 몇 분
  안에 끝나는 작업에만 쓰고, 상한 초과가 반복되면 턴 종료 + reply로 전환한다.
- 턴 종료 + reply: 발주자는 턴을 끝내고, 수행 thread가 parent thread로 terminal을
  직접 보고한다. 장시간 작업 기본값이다.

terminal은 COMPLETED|BLOCKED|NEEDS_USER 하나와 증거(exact changed paths, 실행한
검증 명령·결과, commit/HEAD)를 담는다. 메시지 수신·thread 상태·transport metadata는
완료 증거가 아니다.
...
- 턴 종료 + reply 모드로 발주한 발주자는 턴을 끝내기 전 one-shot heartbeat를
  arm한다. reply를 받으면 대기 중인 heartbeat를 삭제한다.
- wake 턴은 자기 automation을 먼저 삭제하고, task transcript와 Git 상태를
  read-only로 다시 판정한다. unresolved 작업이 남을 때만 새 one-shot을 하나 arm한다.
...
- 아카이브 책임은 발주자(스포너)에게 있다. 각 thread는 자기가 만든 자식만
  아카이브하고, 남의 자식은 아카이브하지 않는다.
