...
완성 후보에서 assembly owner worker가 fresh final reviewer를 한 번 호출한다. 고위험 결정과 복합 spec 서사는
아래 조건부 reviewer만 앞단에서 실행한다.

| Gate | Scope | Primary | Model | Effort |
|---|---|---|---|---|
| implementation worker | every implementation slice, implementation + local checks + WHY commit | fresh local `impl-worker` subagent | `gpt-5.6-luna` | `max` |
...
| impl checkpoint (선택된 high-risk slice만) | owning worker: attempt 1 full once; repair scoped | fresh local `impl-checkpoint-reviewer` subagent | `gpt-5.6-sol` | `medium` |
| final impl-novelist | assembly owner worker: attempt 1 full | fresh local `impl-novelist` subagent | `gpt-5.6-sol` | `medium` |
| final impl-novelist | assembly owner worker: repair attempt 2+ scoped | fresh local `impl-novelist-scoped-reviewer` subagent | worker session 상속 | worker session 상속 |
...
custom-agent TOML은 `gpt-5.6-sol`과 `medium`을 선언해 구현 worker 세션의 설정과
...
declared commands, `assembly_owner`, `final_review_required`, base/prior ancestry를
self-contained packet으로 보낸다. Worker는 편집·로컬 검증·WHY commit을 소유하고, 선택된
high-risk checkpoint와 assembly owner의 final `impl-novelist` 및 repair/scoped reverify도
직접 호출한다. Worker는 raw `impl-worker-report`에 commit SHA, ancestry, reviewer terminal,
artifact path/hash, dirty/index 결과를 반환한다. 부모의 `IMPLEMENTED` 소비는 semantic
acceptance가 아니라 commit 경계·artifact·hash·dirty/index·다음 routing의 seam 확인이다.
...
Final attempt 1은 assembly owner worker가 요구사항/범위, caller·producer·consumer·실패경로, 품질/부채,
...
assembly owner worker가 requirements/scope, impact/quality, fresh commands/full regression,
actor/user-story narrative를 함께 소유한다. 상위 controller는 그 artifact의 terminal/hash와
commit ancestry만 확인한다. `plan-verify`와 routine per-slice `impl-verify` artifact는
