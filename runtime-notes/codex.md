...
Codex는 plan 승인 뒤 별도 `plan-verify`를 자동 호출하지 않고, slice commit마다
`impl-verify`를 만들지 않는다. 작성자 self-review, deterministic plan preflight,
구현 중 명령을 거친 뒤 fresh final reviewer를 한 번 호출한다. 고위험 결정과 복합
spec 서사는 아래 조건부 reviewer만 앞단에서 실행한다.
...
narrative를 함께 소유한다. `plan-verify`와 per-slice `impl-verify` artifact는 Codex
lifecycle의 완료 조건이 아니다.
