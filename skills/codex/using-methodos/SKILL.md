...
name: using-methodos
description: |
  Methodos — this workspace's distributed AI-coding harness (an umbrella name, no central router). Codex uses grill-me for intent, plan for decomposition, Luna/max impl-workers for implementation and commit ownership, worker-owned selective high-risk checkpoints, conditional spec/decision reviewers, and one assembly-owner final impl-novelist verification gate. This meta-skill explains the framework; it does not route work on the gates' behalf.
...
| `impl` | plan approved, 모든 구현은 fresh `impl-worker`(Luna/Max)가 구현·검증·WHY 커밋을 소유하고 부모는 seam만 수용 | worker git commit (`WHY:` prefix) |
...
직접 작성하지 않고 fresh `impl-worker`(gpt-5.6-luna/max)가 작성·로컬 검증·WHY 커밋을
소유한다. 고위험 slice의 Sol/medium checkpoint도 worker가 호출하고, 전체 구현을 한
worker가 조립하면 그 worker가 fresh final `impl-novelist`까지 호출한다. 여러 worker 결과를
합치면 assembly owner worker가 한 번만 final을 호출한다.

기획/오케스트레이션 세션은 WHAT, slice boundary, model route, 최종 candidate ancestry만
소유한다. 상위 Sol/controller는 semantic review를 반복하지 않고 commit 경계, reviewer
terminal, artifact/hash, dirty/index, 다음 routing만 기계적으로 확인한다. spec review
...
fresh local read-only subagent다. 해당 Luna/max worker가 호출한다. Checkpoint는 대상
