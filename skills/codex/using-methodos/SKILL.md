...
name: using-methodos
description: |
  Methodos — this workspace's distributed AI-coding harness (an umbrella name, no central router). Codex directly executes truly simple closed changes; delegated work uses the Luna/high route selected by impl, with one implementation owner per slice and controller-owned selective checkpoint/final review. This meta-skill explains the framework; it does not route work on the gates' behalf.
...
| `impl` | 닫힌 저위험 작업은 execution packet으로 direct/worker 실행; 그 외 slice마다 fresh `impl-worker`가 Luna/high 또는 증거 기반 max로 구현·검증·WHY commit | WHY commit + `impl-worker-report` |
| high-risk slice checkpoint (`impl` 내부) | controller가 schema/public contract, authority/security, persistent/latest/idempotency/concurrency, migration/external state, financial execution, 또는 2개 이상 후속 slice 기반일 때만 fresh 호출 | `<verify_root>/checkpoint-<slug>-slice-<N>-attempt-<M>.json` |
| `impl-novelist` (#4) | controller가 assembled candidate의 모든 구현 report를 기계적으로 닫은 뒤 단 한 번 fresh 호출하는 최종 기술+서사 검증 | `<verify_root>/narrative-<slug>-final-attempt-<M>.json` |
...
구현은 slice마다 fresh `impl-worker`가 Luna/high 또는 `impl`의 증거 기반 max로
작성·검증·WHY 커밋을 소유한다. 이 문서는 그 기준을 복제하지 않는다. controller가
...
slice/assembly owner가 repair하고, controller가 attempt 1 reviewer의 같은 thread/session에
...
- **격리 reviewer** — 중요한 결정의 `decision-reviewer`, spec 누락의 lightweight
  ...
  `impl-novelist`만 별도 컨텍스트로 실행한다. Codex reviewer 호출은 controller가
...
High-risk checkpoint attempt 1과 Codex final attempt 1은 controller가 Sol/medium으로
고정해 직접 호출하는 fresh local read-only subagent다. Checkpoint는 대상 slice마다
full 1회만 실행한다. BROKEN repair는 같은 owner가 수행하고 controller는 동일 reviewer
thread/session에 scoped follow-up만 보낸다. 사용자가 `검토 1회만`이라고 제한하면
scoped follow-up을 생략하고 잔여 위험을 기록한다.

Final gate는 controller가 조립된 candidate의 요구사항/범위,
...
Codex final attempt 1은 controller가 Sol/medium으로 고정해 호출하는 fresh local
