...
| `impl` | 닫힌 저위험 작업은 execution packet으로 direct/worker 실행; 그 외 fresh `impl-worker`의 Luna high/max 선택은 `impl` 정본을 따름 | WHY commit |
| high-risk slice checkpoint (`impl` 내부) | schema/public contract, authority/security, persistent/latest/idempotency/concurrency, migration/external state, financial execution, 또는 2개 이상 후속 slice 기반일 때만. 단, `approved_plan.slices.length == 1 AND owner_role == assembly-owner AND final_review_required == true`이면 checkpoint를 생략하고 high-risk selector/invariant를 final packet에 전달 | `<verify_root>/checkpoint-<slug>-slice-<N>-attempt-<M>.json` |
| `impl-novelist` (#4) | `final_review_required=true`인 assembled 작업에서만 assembly owner worker가 모든 구현 commit·로컬 check 후 단 한 번 호출하는 fresh 최종 기술+서사 검증 | `<verify_root>/narrative-<slug>-final-attempt-<M>.json` |
...
호출하고, 전체 구현을 한 worker가 조립하면 `final_review_required=true`일 때만
그 worker가 fresh final `impl-novelist`까지 호출한다. 여러 worker 결과를 합치면
assembly owner worker가 같은 조건에서 한 번만 final을 호출한다.

기획/오케스트레이션 세션은 WHAT, slice 또는 execution-packet boundary, model route,
owned commit set/provenance만 소유한다. 상위 Sol/controller는 semantic review를
반복하지 않고 commit 경계, reviewer terminal, artifact/hash, dirty/index, 다음
routing만 기계적으로 확인한다. spec review
approval, M1 결정 리스트와 plan review approval은 사용자가 확인한다. 고위험 plan의
조건부 decision-reviewer가 사용자 체감 delta를 만들면 그 결정만 다시 받는다.
...
- **격리 reviewer** — 중요한 결정의 `decision-reviewer`, spec 누락의 lightweight
  ...
  `impl-novelist`만 별도 컨텍스트로 실행한다. 이 reviewer들은 해당 worker/assembly
  owner가 호출한다. 일반 slice는 로컬 RED/GREEN·선언 검증·diff 범위 확인만 하고
...
High-risk checkpoint attempt 1과 Codex final attempt 1은 Sol/medium으로 고정한
fresh local read-only subagent다. 해당 Luna worker가 호출한다. Checkpoint는 대상
slice마다 full 1회만 실행하고 repair만 scoped 재검증한다. 사용자가 `검토 1회만`이라고
제한하면 checkpoint scoped를 생략하고 잔여 위험을 final gate packet에 넘긴다.
단일 slice에서 `owner_role == assembly-owner`이고 `final_review_required == true`인
경우에도 같은 방식으로 checkpoint를 생략하고 trigger reason, linked invariant,
impact selectors, targeted commands, residual risk를 final gate packet에 넘긴다.

Final gate는 assembly owner가 요구사항/범위, caller·producer·consumer·실패경로, 품질/부채,
테스트 oracle·회귀의 4개 기술 lens와 actor/user-story narrative overlay를 한 번에
소유한다. 별도 final full `impl-verify`를 앞뒤로 붙이지 않는다.
