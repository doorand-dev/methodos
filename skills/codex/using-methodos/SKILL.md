...
name: using-methodos
description: |
  Methodos — this workspace's distributed AI-coding harness (an umbrella name, no central router). Codex uses grill-me for intent, plan for decomposition, impl for execution and selective high-risk slice checkpoints, conditional spec/decision reviewers, and one final integrated impl-novelist verification gate. This meta-skill explains the framework; it does not route work on the gates' behalf.
...
- **격리 reviewer** — 중요한 결정의 `decision-reviewer`, spec 누락의 lightweight
  ...
  `impl-novelist`만 별도 컨텍스트로 실행한다. 일반 slice는 로컬 RED/GREEN·선언
  검증·diff 범위 확인만 하고 reviewer를 spawn하지 않는다.
...
fresh local read-only subagent다. Checkpoint는 대상 slice마다 full 1회만 실행하고
repair만 scoped 재검증한다. 사용자가 `검토 1회만`이라고 제한하면 checkpoint scoped를
생략하고 잔여 위험을 final gate packet에 넘긴다.

Final gate는 요구사항/범위, caller·producer·consumer·실패경로, 품질/부채,
