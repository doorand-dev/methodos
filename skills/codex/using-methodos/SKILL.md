...
데이터/사용자 자산·비가역 변경·미해결 WHAT 결정이 모두 없으면 spec·정식 plan·
plan-verify를 만들지 않는다. 같은 턴에 최소 수정, 관련 검증 1개 이상, 프로젝트
...
| `plan-verify` | 작은 plan 자동 생략 조건이 아닌 plan approved 직후, impl 전 | `<verify_root>/plan-<slug>-..json` |
...
Codex의 full reviewer 기본 route는 현재 부모 세션의 model/effort를 상속한 fresh
local read-only subagent이고, 같은 lineage의 fix reverify는 fresh local
Sol/medium scoped다. ChatGPT Pro나 Claude Fable/Opus 같은 외부 reviewer는 사용자가
