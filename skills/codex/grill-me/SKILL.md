...
- **쉬운 언어 강제**: 사용자에게 묻는 문장은 내부 세부값(`verification.type`, `dispatch`, `schema`, `hook`, `artifact`, `reviewer_mode`, `M1/M2` 등)을 먼저 내세우지 않는다. `impl-verify`처럼 사용자와 공유 중인 작업 용어는 써도 되지만, 먼저 "사용자가 무엇을 보게/잃게/다시 하게 되는지"를 한 문장으로 말하고 필요할 때 괄호로 붙인다.
...
spec self-review 후, **신규 기능이 다파일 ∨ 다flow(복합 실사용 경로)**면 `spec-novelist` agent를 dispatch한다 (자명한 1-2파일·단일 flow는 skip — *이 상황 신호*로 게이트가 판단).
...
1. `spec-novelist` agent
   ...
   - 자세: 적대 reviewer와 다른 **순진-생성** (없는 것·못 쓰는 것을 잡음, plan-verify-reviewer가 못 잡는 자리)
...
/plan
   ...
   ↓ BLOCKED → plan SKILL 자동 수정 (N=3 한계 (벤치마크 차용 — 후속), D13)
