...
reasoning effort를 명시한다. 직전 reviewer의 model/effort는 상속하지 않는다.
기본 full은 `model`/`model_reasoning_effort`를 생략한 fresh
`impl-verify-reviewer`가 현재 부모 세션 값을 상속하고, scoped는
`impl-verify-scoped-reviewer(gpt-5.6-sol/medium)`을 쓴다. project machine route가
있어도 외부 Pro/Claude는 사용자가 그 검토에 명시적으로 요청했을 때만 실행한다.
local reviewer를 실행할 수 없으면 자동 외부 fallback 없이 `NEEDS_CONTEXT`다.
explicit full escalation predicate가 있는 attempt M+1만 full route로 보낸다.
verify artifact에는 approved plan revision, current/parent candidate SHA,
full/scoped scope, 실제 provider/transport/model/effort/session을 기록하고 Codex
`fallback_reason`은 null로 둔다.
