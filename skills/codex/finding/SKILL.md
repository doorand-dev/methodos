...
| friction | AI가 코드를 왜 잘못 읽었나 | `.Codex/friction.md` | **휘발**(처리되면 삭제) |
| todo-ctx | 이 todo의 작업 맥락 | `.Codex/todo-ctx/NNN.json` | **휘발**(todo 닫히면 삭제) |
...
| 프로젝트 보완 | `<workspace>/.Codex/finding.toml`의 `extra_triggers` |
...
## 프로젝트 보완 (`<workspace>/.Codex/finding.toml`, 선택)
...
- finding 기록 직전 → [1C] 누더기 거부(발견 합리화·중복 박기 방지) + [3J] 추출 시점(졸업 분기: 휘발 todo-ctx → 영속 finding은 *이전*이 가치 생길 때만)
