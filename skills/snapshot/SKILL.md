...
description: >
  ...
  post-compact Claude의 working context에 이 항목들을 **다시 강제 주입**하는
  도구다.

  /compact는 확률적 요약이라 중요한 항목을 흐릿하게 만들거나 빠뜨릴 수 있다.
  snapshot은 사람·AI가 큐레이션한 명시 우선순위 리스트라서 압축 영향을
  받지 않고 그대로 살아남는다. 압축 후 Claude가 이 파일을 다시 읽으면 그
...
```powershell
python C:\Users\hjcha\.claude\skills\snapshot\scripts\write_context_snapshot.py --input snapshot.json --workspace . --session-id <task-id>
```
