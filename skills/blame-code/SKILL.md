---
name: blame-code
description: AI(또는 사람)가 코드/문서를 잘못 읽은 순간을 코드 구조에 귀책시켜 .claude/friction.md에 한 줄씩 영속 누적. **명시 트리거 전용** — 사용자가 "blame code", "blame-code", "/blame-code", "코드 탓해봐" 라고 직접 말할 때만 발동. 자동 감지 안 함(교정 발화에 무분별 트리거되어 노이즈 누적되는 것 방지). AI 혼란을 코드 결함 신호로 변환하는 3-layer trio의 수집기 — gc(감사)·improve-codebase-architecture(처리)와 연계.
---
...
| 명시 호출 (수동) | `/blame-code` |
| 프로젝트별 보완 | `<workspace>/.claude/blame-code.toml`의 `extra_triggers` |
...
`<workspace>/.claude/friction.md` 없으면 헤더·카테고리 enum·`code-clean` 메타 설명·예시 1개 포함해 생성. 이미 있으면 append만.
...
### 3. 3-agent 판정 파이프라인 (자기 판정 합리화 차단, 관련 설계 결정)

`code-clean`은 controller(=Claude 본인)가 결정하지 않음 — 격리 agent 3개에 위임.
...
호출 실재가 본체. 관련 설계 결정이 막으려던 self-judge 우회가 바로 이 자리).
...
- 직전 사용자 교정 발화 + 그 전 Claude 추론 양쪽을 한 줄로 압축
...
[1D] 단일 정본 — `.claude/friction.md` 헤더의 enum 표를 참조. 본 스킬에선 복사 X.
...
- **controller가 `code-clean` 스스로 판정** (자기 판정 = 관련 설계 결정이 차단한 원래 결함). 항상 3-agent 위임
...
## 프로젝트 보완 (`<workspace>/.claude/blame-code.toml`, 선택)
