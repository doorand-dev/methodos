...
| codex advisory (impl 내부) | 사용자가 Codex 검토를 명시 요청한 경우, *마무리 직전* 1회 | `.claude/verify-reports/codex-impl-<slug>.json` |
...
| **M** | 4-10·결정2-4·flow有 | grill-me + plan + plan-verify + impl + impl-verify + (조건부) decision-reviewer |
| **L** | >10 or architectural/security or 결정≥5 | 풀 + decision-reviewer + impl-verify THOROUGH + narrative #2/#4 |
...
## FORCE vs OPEN (거버닝 원칙 — 모든 게이트 설계를 지배)
...
라우터의 중앙 통제를 버린 대가는 *게이트가 제때 안 켜질 수 있음*이다. 완화: (1) 게이트 description의 HARD-GATE 강제어, (2) 위 영속 artifact 2차 방어, (3) 이 메타-doc의 description이 always-loaded 리마인더로 게이트 존재를 환기. **수용하되 실사용로 mis-fire 관측**.
