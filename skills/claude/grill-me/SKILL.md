...
- **인터뷰 skip ≠ novelist skip** (실사용 실측 누락점, todo-ctx/008): 대화 합성 모드여도 신규 기능이 다파일 ∨ 다flow면 §6b `spec-novelist` 1회 dispatch는 *그대로 발동*. "사용자와 이미 충분히 얘기함"은 *인터뷰* 면제 근거지 *fresh-context 순진-사용 서사* 면제 근거가 아니다 — 둘은 별개 조건(전자는 intent 도출, 후자는 main-session이 못 보는 빈칸 노출). spec frontmatter `novelist.required:true`면 `status:done` 전엔 plan이 거부(§ plan preflight)
...
> spec-novelist는 구현 논의를 모르는 실사용자 관점으로 spec만 읽고 user_story를 prose로 걸어 missing_flow / ambiguity / module_gap을 찾는다. 결과는 `novelist` frontmatter와 spec 보완으로 fold한다.
