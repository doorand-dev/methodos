...
프로젝트 루트 `.claude/todos.md` 영속 할 일 관리. *설계·WHY는* → [METHODOLOGY.md](METHODOLOGY.md).

> 관련 설계 결정 — todo = 유일 작업 layer. handoff·ADR이 backlink로 가리킴.
...
>       맥락 3줄+ 넘으면 `.claude/todo-ctx/NNN.json` 사이드카로 (줄은 린 유지).
>       관련 todo 묶음 = 줄에 공유 토큰(예 `(roi8)`) — grep로 묶고 [x] 세서 진척.
> 정리: "todo 정리" → `[x]` 전부 하단 `## Archive`로 한 줄 tombstone으로 이동(삭제 X — backlink grep 생존).
>       이 archive 시점에 lean화: 여러 줄·사이드카 잔재 버림 — lean 규칙 전 fat 항목도 *닫힐 때* 자동 린화(사전 일괄전환 X). 영속 지식은 ADR/finding 졸업 먼저.
...
**규약은 SKILL.md가 아니라 이 헤더에 산다** — todo 쓸 때 보는 건 todos.md 그 파일이라 헤더 규약만 실제로 따라짐 (point-of-use, 관련 설계 결정).
...
3. **완료**: `- [ ]` → `- [x]`. id 유지.
4. **삭제**: 가급적 X — `closes todo #NNN`·ADR·handoff가 `#NNN` 역참조. 진짜 필요시만 `## Archive`로.
5. **정리** ("todo 정리" 발화 시 자동): `- [x]` 전부 하단 `## Archive`로 **한 줄 tombstone**으로 이동. **이 archive 시점에 lean화** — 여러 줄·사이드카 잔재는 여기서 버림. ∴ lean 규칙 전 fat 항목도 *닫힐 때* 자동 린화되니 **사전 일괄전환 불필요**(open 동안은 fat 둠 — 작업 맥락 살림, churn 0). 영속 지식 있으면 ADR/finding 졸업 먼저. mission-control·gc·trio-load 전부 Archive 인식 → 무충돌. (규약은 todos.md 헤더 — point-of-use)

## 보류 발화 1턴 대기 (관련 설계 결정)
...
- `.claude/` 없으면 생성. `todos.md` 없으면 위 헤더(규약 박스 포함)로 lazy 생성.
...
**[METHODOLOGY.md](METHODOLOGY.md)** (관련 설계 결정/[0019]/[0020]/[0026]).
