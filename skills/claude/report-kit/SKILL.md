...
name: report-kit
description: |
  생애주기 HTML 양식 키트 — 프로젝트 진행·데이터흐름·측정·판정을 자기완결 단일 HTML 한 장으로 시각화할 때 쓰는 공유 팔레트·컴포넌트·아키타입. 7 아키타입(Spec검토·시나리오흐름·시스템전경·생애주기매트릭스·측정스냅샷·통제A/B diff·판정폼)을 3 family로. 명시 트리거 위주(자동 남발 금지): "/report-kit"·"report-kit"·"이 키트로"·"이 양식으로 만들어"·"생애주기 HTML". 위 양식 중 하나를 새로 만들 때 consult — 매번 CSS 재발명 금지. prototype v0.
---

# report-kit (prototype v0)

프로젝트 상태·데이터흐름·측정·판정을 **자기완결 단일 HTML 한 장**으로 시각화하는 공유 키트.
여러 내부 산출물에서 추출한 시각 DNA를 재사용해, 매번 CSS를 재발명하지
않고 *한 가족으로 보이는* 양식을 빠르게 찍는다.

> **STATUS: prototype v0 — 진화 전제.** 실사용 마찰을 쌓아 evolve. 지금은 *쓰면서 배운다* — 보류.
...
골격 코드 + 판단 셀 손 = 하이브리드. B·C는 빌더가 `data→html`(빌더 스크립트가 source, HTML은 산출).
...
4. **provenance 푸터 필수** — `생성일 · source file · 근거`. 손작성(A형)이면 "변경 시 손수 갱신" 명시.
...
- **OneDrive 공유 (공유 산출)**: AI 산출물은 OneDrive 루트 말고 **프로젝트별 폴더** `<공유 폴더>/<프로젝트>/`에, 파일명 `<설명>-<날짜>.html` (폴더 없으면 만들어 그 안에). 루트·플랫이 난잡해져 사용자가 못 찾는 것 방지. 필요하면 채팅 전달을 병행.

## Codex·외부 실행자 위임

데이터형(B)·루프형(C)은 빌더라 Codex 적합(기계적·명확스펙). Codex는 같은 PC에서 직접 돌리므로
임베드 프롬프트 말고 **이 skill folder를 주고 읽게** 한다 —
...
실사용 마찰을 누적하다 정합 판단이 서면: 반복되는 빌더 패턴 2종(B형 `data→html`, C형 round-trip)을 재사용 스크립트로 추출하고, skill 본문과 `kit.html` 예시를 함께 갱신한다.
