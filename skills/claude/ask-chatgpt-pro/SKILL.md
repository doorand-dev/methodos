...
3. On completion, **collect through the mechanical gate — never read `.session.answer` directly.** The background watcher's completion notification can fire *during* a thinking pause, so a fresh re-activation does NOT by itself mean the answer is done. The `collect_final` function below writes ChatGPT's answer to **stdout only when the session is truly final** (status `complete` + `completedAt` set + answer non-empty + length stable across a 4 s re-read); otherwise stdout is empty and the reason goes to stderr. **Empty stdout ⇒ nothing to quote** — `watch` again, then re-run it. It uses `node` (never jq/python/temp files: jq is often absent on Windows and agbrowse *is* a Node CLI, so `node` is always present); it also handles the shape trap that `sessions show` nests the answer under `.session.answer` while `sessions list` is flat `.answer`.
   ...
   collect_final(){ SID="$SID" node -e '
     ...
     const MIN=Number(process.env.MIN_ANSWER_CHARS||40);
     ...
     process.stderr.write(`gate read1: ${a.status} len=${(a.answer||"").length} completedAt=${a.completedAt||"-"} min=${MIN}\n`+
                          `gate read2: ${b.status} len=${(b.answer||"").length} completedAt=${b.completedAt||"-"} min=${MIN}\n`);
     const final=b.status==="complete"&&b.completedAt&&(b.answer||"").length>=MIN&&(a.answer||"").length===(b.answer||"").length;
     if(!final){process.stderr.write("NOT FINAL — preamble, still streaming, or a stable answer shorter than the MIN_ANSWER_CHARS floor. Do NOT quote. watch again, then re-run collect_final.\n");process.exit(3);}
...
4. **Why the gate exists — do not weaken it.** Pro extended has long "thinking" pauses; `watch`/`poll` can fire terminal *during* such a pause and hand back only the intro (e.g. an answer that stops at "새 결함만 추리겠습니다"). `.lastStreamingState` is unreliable — it reads `"unknown"` even when genuinely complete (verified 2026-07-01) — so **do NOT trust it**, and do not treat a short first reply as the answer just because a notification arrived. A length-stable answer is *also* not automatically final: the gate enforces a minimum-length floor (`MIN_ANSWER_CHARS`, default 40) so a short-but-stable preamble (e.g. "네, 검토하겠습니다.") can't slip through just because two reads agree — a stable answer shorter than the floor is treated as a preamble, not a finished reply; override with `MIN_ANSWER_CHARS=<n>` for a genuinely short expected answer. The gate's two `gate read…` stderr lines are your visible trace (now printing `min=…` too); if it exits NOT FINAL, keep the session and `watch` again rather than quoting a preamble. Mechanically withholding the text on stdout (not prose you must remember to obey) is the whole defense against the "short first reply mistaken for the answer" failure.
