...
description: |
  ...
  **Standalone — 사용자 결정 공간** ([ADR 0004](../../../../docs/adr/0004-conv-plan-outside-goal.md)). spec.md 입력으로 받아 *추가 인터뷰 없이 합성*. 산출 `.claude/plans/slug.md` approved 후 model-driven 자율주행으로.
  **right-sizing (OPEN)**: 슬라이스 두께·P0 스파이크 유무는 상황으로 *모델 판단* (rigid rule 아님, [ADR 0022] FORCE/OPEN). 명시: `/plan slug 또는 거친 골`.
---
...
> - (위 한 줄로 평시엔 충분. 원칙 11개 전체 정의가 필요할 때만 심화 참조: [decision](../../../skills/decision/SKILL.md).)

## 트리거 (self-trigger — 라우터 없음, [ADR 0022])
...
### Frontmatter schema ([ADR 0006](../../../../docs/adr/0006-conv-grill-me-package.md))
...
3. **슬라이스 분해** ([3H] vertical, thin 우선):
   ...
   - **교체 vs 증분 prime** (산입 바이어스 차단): AI는 새것을 *추가*만 하고 그게 대체하는 옛것(죽는 함수·도달 불가 분기·중복 산문)을 잔류시키는 편향이 있다. `files.modify`가 있는 슬라이스마다 *한 번* 자문 — **"이 슬라이스는 기존을 *교체*하나, *증분 추가*하나?"** 교체면 본문 Steps에 *삭제 대상*을 명시 step으로 적어 impl이 추가와 함께 지우게 한다(예: `- [ ] Step N: 옛 update_v1() 및 호출부 제거`). 증분이면 표시 안 함. ***검증 필드 아님*** — plan-time 행동 prime일 뿐(선언된 인식이 있으면 대개 삭제도 따라옴). 선언 안 된 잔재의 사후 catch는 [gc](../../../skills/gc/SKILL.md) 도달성 감사 담당. **의식화 금지** — `files.modify` 없는 순수 create 슬라이스엔 안 물음(routine modify에 빈 자문 남발 X).
...
11. **M2 scenario delta approval** (조건부):
    ...
    **분기**:
    - *체감 변화 없음* → 사용자에게 **한 줄 알림 + escape** (D34, ADR 0012 F2):
      ```
...
agent가 plan 받으면 *interface symbol 모른 채* re-derive 부담 → signature·schema는 inline (paste-to-reviewer 같은 세션 모델이라 stale 우려 무, [ADR 0004]).
...
> **impl-verify 오라클 타입(G-B)과 정렬** ([ADR 0022](../../../../docs/adr/0022-conv-methodos-distributed-gates.md)): 위 type ↔ impl-verify 오라클 택소노미 대응 — `unit_test`↔tdd-parity · `command`↔live-dry-run · `artifact`↔spike-measurement · `visual`↔visual(G-C) · `custom`↔adversarial-review 등. impl-verify가 슬라이스 오라클 타입을 판정할 때 이 `verification.type`을 1차 신호로 읽는다 (두 분류 체계 일치).
...
- plan을 뒷단 자율주행 전, 사용자 결정 공간에서 작성 — /plan은 사용자 결정 공간이 정본 ([ADR 0004])
...
- /plan을 사용자 결정 공간에 둔 게 *사용자 부담* 너무 크면 → ADR 0004 Reeval (조건부 자율주행 공간 내부 모드 복귀)
...
- **교체 vs 증분 prime이 의식화**(modify 슬라이스마다 빈 "증분입니다" 남발) 또는 *prime에도 삭제 누락이 그대로*(gc가 잡는 잔재 비율 안 줄어듦, 2026-09까지 무효과) 발견 시 → 제거. prime은 *soft 행동 유도*라 효과 측정 어려움 — 무효·의식화 양방향 다 제거 트리거. (catch로 격상은 sentinel_dead_code 기각 근거상 금지 — [ADR 0023](../../../../docs/adr/0023-conv-adopt-evidence-check-hook.md).)
